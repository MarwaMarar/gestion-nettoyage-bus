...
export class ListeNettoyagesWorkflow implements OnInit {
  values:Nettoyage[]=[];error='';
  constructor(private service:NettoyageService,private router:Router,private cdr:ChangeDetectorRef){}
  ngOnInit():void {this.service.enAttente().subscribe({next:values=>{this.values=values;this.cdr.detectChanges();},error:error=>{this.error=error?.error?.message||'Impossible de charger les nettoyages.';this.cdr.detectChanges();}})}
